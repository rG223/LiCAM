...
class Trainer(DefaultTrainer):
    ...
    def run_step(self, ratio, iter):
        ...
        """
        loss_dict, pred_class_logits, gt_classes = self.model(data, ratio, iter)
        ...
        #         pickle.dump(self.record, f)
        self.optimizer.step()
        return pred_class_logits, gt_classes
...
def main(args):
    cfg = setup(args)
    # cfg.TEST.EVAL_PERIOD = 6212
    if args.eval_only:
        model = Trainer.build_model(cfg)
        DetectionCheckpointer(model, save_dir=cfg.OUTPUT_DIR).resume_or_load(
            cfg.MODEL.WEIGHTS, resume=args.resume
        )
        res = Trainer.test(cfg, model, ratio=None, iter=None)
